...
impl<'a> ComputationGraph<'a> {
    ...
    pub fn add_operation(&mut self, op_type: OperationType, inputs: Vec<*const c_void>, grid_dim: dim3, block_dim: dim3) {
        self.operations.push(Operation { op_type, inputs, grid_dim, block_dim});
    }
        ...
        for op in &self.operations {
            match &op.op_type {
                OperationType::Kernel(kernel_name) => {
                    self.cuda_context.launch_kernel(kernel_name, op.grid_dim, op.block_dim, &op.inputs, "stream1")?;
                }
